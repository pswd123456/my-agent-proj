...
- `edit_file` 承接局部修改，模型侧提供 `oldString` / `newString`，工具内部生成统一 diff；已有文件修改前要求本 session 内先通过 `read_file` 读取且文件未变化。
...
`write_file` 已切到 atomic write helper；`edit_file` 承接局部修改并对已有文件做 fresh read / stale check，改动结果仍通过统一 diff 层保存。
...
- 为 `write_file` / `edit_file` 引入 atomic write / fresh read / stale check。
...
- `packages/agent/src/tools/unified-patch.ts`

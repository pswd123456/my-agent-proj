...
- P1：`write_file` 已切到 atomic write helper，并收敛为新建文件与整文件替换；`apply_patch` 承接行级和多文件修改；已有文件写入或补丁修改前都会校验本 session 内最近一次 `read_file` 的文件版本
...
- 为 `apply_patch` 补齐已有文件修改前的 session read / stale check
- 为写路径相关工具补齐高风险回归测试
...
- `write_file` 和 `apply_patch` 修改已有文件前，必须发现缺少 session 内 `read_file` 或读后文件变化的 stale 场景

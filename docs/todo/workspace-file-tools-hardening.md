...
- P1：`write_file` 已切到 atomic write helper，并统一承载整文件替换与行范围编辑；已有文件写入前会校验本 session 内最近一次 `read_file` 的文件版本
...
- 为 `write_file` 引入 atomic write helper，并统一整文件替换与行范围编辑
- 为写路径相关工具补齐高风险回归测试
...
- `write_file` 修改已有文件前，必须发现缺少 session 内 `read_file` 或读后文件变化的 stale 场景

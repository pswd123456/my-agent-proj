...
- P1：`write_file` 已切到 atomic write helper，并收敛为新建文件与整文件替换；`edit_file` 承接局部修改并在内部生成统一 diff；已有文件写入、编辑或删除前都会校验本 session 内最近一次 `read_file` 的文件版本
...
allowWorkspaceEscape =
  input.allowWorkspaceEscape ?? tool.sandboxProfile === "workspace-rooted";
```
...
- 为 `write_file` 引入 atomic write helper，并收敛为新建文件与整文件替换
- 为 `edit_file` 补齐已有文件修改前的 session read / stale check
- 为写路径相关工具补齐高风险回归测试
...
- `write_file` 和 `edit_file` 修改已有文件前，必须发现缺少 session 内 `read_file` 或读后文件变化的 stale 场景
...
- secret redaction 本轮仍不做实现，当前只补 binary / device / output guard 与 loop guard；若后续需要敏感信息脱敏，单独起规格，不和本轮工具契约混做

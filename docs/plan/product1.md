...
先做日程管理方向，交付一个可以把用户的自然语言安排转成结构化日程、在无冲突时自动落库、在有冲突时请求用户确认、并支持后续查询与修改的最小产品闭环。
...
用户输入一段自然语言的日程意图，agent 将其整理成当天或指定日期的结构化安排；如果没有冲突就直接写入数据库，如果有冲突再向用户发起确认，并支持后续查改删。
...
因此产品的第一价值，不是“做一个日历 CRUD 界面”，而是“把自然语言安排转成可自动执行、冲突时可确认、可编辑、可持久化的结构化日程”。
...
> 22:00-07:00 sleep  
>
> 但这里和你现有的 14:00-15:00 routine 冲突。  
> 是否允许我覆盖 / 改排这段时间？
...
只有当出现冲突、覆盖或明显歧义时，agent 才需要向用户确认后再调用写入类 tool。
...
对于 `this morning`、`around 2pm` 这类模糊表达，agent 可以先做合理推断；如果推断后的安排无冲突，可以直接落地，但返回文本里必须体现默认推断；如果推断涉及冲突或覆盖，则必须进入确认流程。
...
用途：当出现冲突、覆盖风险或高歧义场景时，把候选日程整理成纯文本确认消息，等待用户反馈。
...
- existing: 2026-04-21 14:00-15:00 dentist
- proposed: 2026-04-21 14:00-15:00 meeting
...
-> if conflict: ask_for_confirmation
...
“用户说一段自然语言，agent 能在无冲突时自动安排并落库，在有冲突时请求确认。”

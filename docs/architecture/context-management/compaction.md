...
- 更早的 `user`、`assistant` 文本与 `assistant thinking` block 原样保留，不做摘要或截断
- 更早的 tool block（tool call、tool result）按原始顺序就地替换成 synthetic `user` summary block
- tool summary 不包含 tool result 正文，只保留工具名、成功/失败状态与输出长度
- 第一个 synthetic block 的 id 是 `history-compaction-summary`，后续连续摘要块使用同前缀加序号

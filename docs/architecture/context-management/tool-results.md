...
工具执行完成后，runtime 会通过 `persistToolActionCompletion()` 把工具返回的 `result.content` 原样写入 `session.messages` 的 `tool result` block，并在 trace 中记录同一份输出。单工具路径的 `executeToolAction()` 也复用这条持久化逻辑。
...
- 并发安全工具并行执行后，tool result 仍按原始 tool call 顺序写回
- 工具错误结果能给模型下一步可修正信息

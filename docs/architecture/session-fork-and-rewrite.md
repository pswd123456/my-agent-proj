...
5. 如当前 session 绑定了 task brief，则把 brief 复制到 fork session 自己的 `.agents/plans/<sessionId>/`
...
这里有一个额外收口：不是所有 assistant message 对应的 checkpoint 都能直接暴露成 fork target。只有“该用户回合真正收束后的 final response”对应的 assistant message 才允许 fork；如果那条 assistant 文本后面同一回合里还跟着 tool call、tool result 或其他过程块，它就只算过程消息，不算可 fork 的历史结束点。
...
- 不把中间过程 assistant message 伪装成可 fork 的 final response

...
- [Session Fork 与 Rewrite](./session-fork-and-rewrite.md)
...
- 如果想确认哪些状态进全局/工作区 TOML、哪些进 session、哪些进后台任务表，读 [持久化与 Session 状态模型](./persistence-and-session-state.md)
- 如果想确认历史 assistant 节点为什么能 fork、rewrite 为什么只允许最新一轮，以及 checkpoint / replay / rewind 的边界，读 [Session Fork 与 Rewrite](./session-fork-and-rewrite.md)

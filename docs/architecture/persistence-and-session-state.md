...
- `userId` 复用现有 settings / session 默认值链路
- `settings.responseOutputMode` 控制只输出 final answer 还是输出进度摘要
- `lastUpdateId` 用来忽略重复 Telegram update

这层不保存聊天全文，不做队列，也不替代 `agent_sessions` / `session_messages`。Telegram 普通消息进入后仍由当前 session runtime 执行；如果 active session 正在 running，adapter 会报错并丢弃新消息。
...
- inbox binding 持久化：`packages/db/src/inbox-repository.ts`

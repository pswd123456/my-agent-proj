...
这条链路负责把“当前会话之外的执行”单独拆出来，并统一走一层 background task orchestration：
...
- `packages/agent/src/background-tasks/orchestration.ts`：封装后台任务 poll/wakeup 调度与 active task 聚合
- `packages/agent/src/background-tasks/runner.ts`：执行 claim 到的 detached task，并把等待用户输入 / 等待主 agent 回复 / shell 完成结果回写给任务层
...
sequenceDiagram
  ...
  Main->>DB: enqueue shell_command task
  Worker->>DB: claimNextTask / heartbeat
  Worker->>Child: load child session or shell process and run
  Child-->>Worker: RunSessionResult / shell result
...
- 任务记录保存执行状态、payload、`taskState` 和结果摘要
...
- 对这些任务，worker 心跳过期或超过 deadline 后不会静默重试；会直接失败并向主会话注入失败通知

...
async function commandList(options: CliOptions): Promise<void> {
  ...
  for (const filePath of selected) {
    ...
    const turns = buildTurnSummaries(records);
    ...
    console.log(
      `  size: ${sizeKb}KB, events: ${records.length}, segments: ${turns.length}, error_turns: ${errorTurns}`
    );
    console.log(
...
function printOverview(
  ...
  console.log(`Segments: ${turns.length}`);
  ...
  console.log(
    `Recorded turn values by segment: ${turns
...
function printTimeline(turns: TurnSummary[], options: CliOptions): void {
  const visibleTurns = turns.filter((turn) => {
    return isSelectedSegment(turn, options);
  });

  console.log("\nTimeline:");
  if (visibleTurns.length === 0) {
    console.log("  (no turns matched the current filters)");
    return;
  }
  ...
  for (const turn of visibleTurns) {
    ...
    console.log(
      `  Segment ${turn.sequence} (turns ${observedTurnText}): ${formatTime(turn.startedAt)} -> ${formatTime(turn.endedAt)}`
    );
    console.log(
...
function isSelectedSegment(turn: TurnSummary, options: CliOptions): boolean {
  if (
...
function printDetailedEvents(turns: TurnSummary[], options: CliOptions): void {
  ...
  const selected = turns
    .filter((turn) => isSelectedSegment(turn, options))
  ...
  for (const record of selected) {
    const header = `  [turn ${record.event.turnCount}] ${record.event.kind} @ ${formatTime(record.createdAt)}`;
    console.log(header);
    switch (record.event.kind) {
      case "prompt":
...
async function commandInspect(options: CliOptions): Promise<void> {
  ...
  printOverview(records, turns, traceFile, stateDirectory);
  printTimeline(turns, options);
  printDetailedEvents(turns, options);

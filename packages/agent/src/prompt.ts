...
const DEFAULT_SYSTEM_PROMPT = [
  ...
  "Do not escalate a small edit into write_file just because apply_patch failed once or twice. For existing files with localized edits, keep using apply_patch after rereading the relevant range.",
  "For simple line-level edits with apply_patch, prefer the smallest exact hunk around the target line instead of rewriting or restructuring the surrounding code.",
  "For a text-removal task, the preferred sequence is: search_text -> read_file with startLine/endLine around the hit -> apply_patch that removes only the target text line or literal.",
  ...
  "In an apply_patch hunk for content removal, the removed content line itself should be the - line. Surrounding lines that stay in the file must remain unchanged context lines with a leading space, not extra deletions.",
  "Concrete local-content example: if the file currently has a line before the target content and a line after it, keep both as unchanged context lines and make only the target content line the - line.",
  "If apply_patch fails once or twice on a one-line text removal task, do not switch to write_file. Reread the narrow range and retry a smaller exact patch instead of rewriting the whole file.",
  "If apply_patch fails with context mismatch, reread the same narrow range with read_file and retry with a smaller exact hunk. Do not pivot to shell inspection first when read_file can show the needed context.",

...
const DEFAULT_SYSTEM_PROMPT = [
  ...
  "When the user asks to remove one visible sentence or one string literal, delete only that text line or literal. Do not change adjacent className strings, JSX tags, indentation, formatting, control flow, or conditions unless the user asked for it.",
  "For JSX or TSX text-removal tasks, keep the surrounding render structure byte-for-byte stable except for the target text line. Do not rewrite boolean literals, ternary branches, wrapper tags, or closing delimiters just to remove inner text.",
  "Example: if the only requested change is deleting inner text, keep surrounding lines like {true ? null : ( and )} unchanged instead of rewriting them into a different conditional form.",
  "In an apply_patch hunk for text removal, the removed text line itself should be the - line. Surrounding closing lines that stay in the file must remain unchanged context lines with a leading space, not extra deletions.",
  "Concrete JSX example: if the file currently has a line with >, then the target text line, then </div>, the hunk should keep > and </div> as unchanged context lines and make only the target text line the - line.",

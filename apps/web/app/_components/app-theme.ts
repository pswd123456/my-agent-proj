...
import {
  foundation,
  semanticThemes,
  type SemanticThemeName
} from "@ai-app-template/tokens";

export type AppThemeMode = SemanticThemeName;

export const APP_THEME_STORAGE_KEY = "workbench-theme-mode";
...
export function getAppThemeStyle(mode: AppThemeMode): CSSProperties {
  ...
  return {
    ...
    "--app-switch-track-off":
      mode === "day" ? "rgba(75, 60, 46, 0.16)" : "rgba(246, 239, 223, 0.14)",
    "--app-switch-track-on": foundation.color.jade[700],
    "--app-switch-thumb":
      mode === "day" ? foundation.color.sand[50] : foundation.color.sand[50],
...
export function applyAppThemeVariables(
  ...
  const variables = getAppThemeStyle(mode);
